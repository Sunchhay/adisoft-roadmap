...
export function formatCountryFlags(codes: string[]) {
  if (codes.length === 0) return "🌐";
  const visible = codes.slice(0, 3).map(countryFlagEmoji).join(" ");
  return codes.length > 3 ? `${visible} +${codes.length - 3}` : visible;
}
...
export function CountryFlags({ codes }: { codes: string[] }) {
  ...
  return <span className="inline-flex shrink-0 items-center gap-0.5" aria-hidden="true">
    {codes.slice(0, 3).map((code) => <span key={code} className="text-base leading-none">{countryFlagEmoji(code)}</span>)}
